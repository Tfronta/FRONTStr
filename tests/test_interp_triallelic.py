...
def _system(
    allow_tri: bool = False, tri_balanced_thr: float | None = None, name: str = "TPOX"
) -> System:
    return System(
        name=name,
        chromosome="chr2",
        ...
        motif="AATG",
        period=4,
        allow_triallelic=allow_tri,
        tri_balanced_thr=tri_balanced_thr,
    )
...
def test_three_peaks_in_non_triallelic_locus_flags_mixture() -> None:
    ...
    a, rule, tri = call_profile([_allele(0, 60), _allele(1, 55), _allele(2, 50)], sys)
...
def test_tpox_type_ii_balanced() -> None:
    ...
    a, rule, tri = call_profile([_allele(0, 60), _allele(1, 50), _allele(2, 45)], sys)
    assert len(a) == 3
...
def test_tpox_type_i_unbalanced() -> None:
    ...
    a, rule, tri = call_profile([_allele(0, 100), _allele(1, 90), _allele(2, 35)], sys)
    assert len(a) == 3
...
def test_triallelic_review_when_pattern_is_weird() -> None:
    ...
    _a, rule, tri = call_profile([_allele(0, 100), _allele(1, 40), _allele(2, 35)], sys)
...
def test_third_below_calling_thresh_drops_to_het() -> None:
    ...
    a, rule, _tri = call_profile(
        [_allele(0, 100), _allele(1, 80), _allele(2, 5)],
        sys,
        calling_thresh=0.10,
    )
