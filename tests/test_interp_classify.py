...
def _allele(
    *, consensus: str = "AGAT" * 12, n_reads: int = 50, is_deletion: bool = False,
) -> Allele:
    return Allele(
        cluster_index=0, consensus=consensus, length_bp=len(consensus),
        n_reads_total=n_reads, n_reads_hp1=0, n_reads_hp2=0, n_reads_hp_none=n_reads,
        n_forward=n_reads, n_reverse=0, mean_qual=30.0,
        ce=12.0, isfg="[AGAT]12", bp_diff=0, is_deletion=is_deletion,
    )
...
def test_classify_thresholds(n_reads: int, total: int, expected: AlleleStatus) -> None:
    ...
    status = classify_allele(
        a, total_reads=total, expected_stutter={},
        analytical_thresh=0.02, calling_thresh=0.10,
    )
...
def test_classify_stutter() -> None:
    ...
    es = {"AGAT" * 11: 12.0}
    status = classify_allele(
        a, total_reads=100, expected_stutter=es,
        analytical_thresh=0.02, calling_thresh=0.10,
    )
...
def test_classify_deletion() -> None:
    ...
    status = classify_allele(
        a, total_reads=100, expected_stutter={},
        analytical_thresh=0.02, calling_thresh=0.10,
    )
...
def test_classify_no_data() -> None:
    a = _allele(n_reads=0)
    status = classify_allele(
        a, total_reads=0, expected_stutter={},
        analytical_thresh=0.02, calling_thresh=0.10,
    )
